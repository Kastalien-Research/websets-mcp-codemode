...
function formatEvent(payload: LinearWebhookPayload): {
  content: string;
  ...
  if (data.id) meta.issue_id = String(data.id);
  if (data.identifier) meta.identifier = String(data.identifier);
  if (data.teamKey) meta.team = String(data.teamKey);
  ...
  const content = JSON.stringify(
    {
      ...
      title: data.title,
      state: data.state ? (data.state as Record<string, unknown>).name : undefined,
      priority: data.priority,
      assignee: data.assignee ? (data.assignee as Record<string, unknown>).name : undefined,
      labels: data.labels
        ? (data.labels as Array<Record<string, unknown>>).map((l) => l.name)
        : undefined,
...
export function startChannel(): void {
  const mcp = new Server(
    { name: "linear-channel", version: "0.1.0" },
    {
      capabilities: {
      ...
      instructions:
        "Linear events arrive as <channel> notifications. Attributes include: " +
  ...
  mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    switch (req.params.name) {
      case "comment_on_issue": {
        ...
        return {
          content: [{ type: "text" as const, text: `Comment added (id: ${data.commentCreate.comment.id})` }],
        };
      }
      ...
      case "transition_issue": {
        ...
        const target = states.workflowStates.nodes.find(
          (s) => s.name.toLowerCase() === (args.state_name as string).toLowerCase()
        );
        if (!target) {
          return {
            content: [{
        ...
        return {
          content: [{ type: "text" as const, text: `Issue transitioned to "${target.name}"` }],
        };
      }

      case "assign_issue": {
        // Find user by email
        const users = (await linearQuery(
        ...
        if (!users.users.nodes.length) {
          return {
            content: [{ type: "text" as const, text: `No user found with email "${args.assignee_email}"` }],
            isError: true,
          };
        }
        ...
        return {
          content: [{ type: "text" as const, text: `Issue assigned to ${users.users.nodes[0].name}` }],
        };
      }
      ...
      case "create_issue": {
        ...
        const team = teams.teams.nodes.find(
          (t) => t.key.toLowerCase() === (args.team_key as string).toLowerCase()
        );
        if (!team) {
          return {
            content: [{
        ...
        if (args.state_name) {
          const states = (await linearQuery(
            ...
            { teamId: team.id }
          )) as { team: { states: { nodes: Array<{ id: string; name: string }> } } };
          const state = states.team.states.nodes.find(
            (s) => s.name.toLowerCase() === (args.state_name as string).toLowerCase()
          );
        ...
        if (args.labels && (args.labels as string[]).length > 0) {
          const allLabels = (await linearQuery(
            `query { issueLabels { nodes { id name } } }`
          )) as { issueLabels: { nodes: Array<{ id: string; name: string }> } };
          const labelIds = (args.labels as string[])
        ...
        const data = (await linearQuery(
          ...
          { input }
        )) as { issueCreate: { success: boolean; issue: { id: string; identifier: string; title: string; url: string } } };
      ...
      case "list_issues": {
        ...
        if (args.team_key) filters.push(`team: { key: { eq: "${args.team_key}" } }`);
        if (args.state) filters.push(`state: { name: { eq: "${args.state}" } }`);
        if (args.assignee_email) filters.push(`assignee: { email: { eq: "${args.assignee_email}" } }`);

        const filterStr = filters.length ? `filter: { ${filters.join(", ")} }` : "";

        const data = (await linearQuery(
          `query { issues(first: ${limit}, ${filterStr}, orderBy: updatedAt) {
